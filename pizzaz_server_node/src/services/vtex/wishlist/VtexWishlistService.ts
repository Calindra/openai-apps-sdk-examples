...
interface ListItemInputType {
  productId: string;
  title: string;
  sku: string;
}

interface ViewListsResponse {
  data: Array<{ productId: string; sku: string; title: string; id: string }>;
  name: string;
  public: boolean;
}
...
export default class VtexWishlistService {
  ...
  static async listItems(from = 1, to = 20): Promise<ViewListsResponse> {
  ...
  static async removeItem(id: string, name = "Wishlist"): Promise<any> {
    const user = await VtexCustomerService.retrieveCustomerData();
  ...
  static async checkItem(productId: string): Promise<CheckItemResponse> {
    const user = await VtexCustomerService.retrieveCustomerData();
